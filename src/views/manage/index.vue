  ...
  <div class="manage">
      ...
      <el-tab-pane :label="item.name" v-for="item in intercalate" :key="item.id" :name="item.id">
        ...
        <div class="drag-self">
          <div
            ...
            v-for="(item2, index) in storeHouseSet"
            :key="index"
            :id="item2.value"
            draggable="true"
    ...
    <el-dialog title="库房参数设置" :visible.sync="centerDialogVisible1" width="30%" center>
      ...
      <el-row class="cat_opt">
        ...
        <!-- 区域控制器设备 -->
        <el-tag v-for="tag in areaControllerIndexItem" :key="tag.name" closable>{{ tag }}</el-tag>
      </el-row>

      <!-- 库房参数 -->
      <el-form
        v-if="areaControllerIndexItem != null"
...
export default {
  ...
  data() {
    return {
      ...
      storeHouseSet: [
        { ttile: '温湿度一体机', value: 'wenshi' },
        { ttile: '单行文字', value: 'danhang' },
        { ttile: '设置', value: 'shezhi' }
      ],
      // 判断弹出框对其方式
      labelPosition: 'right',
      // 设备参数
  ...
  methods: {
    ...
    addStorehouse() {
      ...
      this.areaControllerIndexItem = null
    ...
    roomFormLabelAlign(formName) {
      if (this.areaControllerIndexItem == null)
    ...
    dragStart(ev) {
      ...
      console.log(ev.target.id + this.intercalate[storeIndex].infoList.length)
      let info = {
        value: ev.target.id + this.intercalate[storeIndex].infoList.length
      }
